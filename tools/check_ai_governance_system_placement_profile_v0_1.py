...
RESTRICTED_AUTHORITY_PATTERNS = [
    ...
    ("ERR_RESTRICTED_COMPLIANT", re.compile(r"\bcompliant\b|\bcompliance[-_\s]*(?:approved|ready|certified|guaranteed)\b")),
    ("ERR_RESTRICTED_LEGAL_APPROVAL", re.compile(r"\blegally[-_\s]*(?:approved|ready|sufficient)\b|\blegal[-_\s]*(?:approval|sufficiency|admissibility)\b")),
...
def normalize_text(value: Any) -> str:
    if value is None:
        return ""
    text = str(value)
    # Normalize compatibility forms and common obfuscating separators.
...
def run_checks(record: Optional[Any], record_path: Path, schema_path: Path) -> List[Dict[str, Any]]:
    ...
    if schema_path.exists():
        checks.append(check("SCHEMA_FILE_PRESENT", "PASS", "Schema file is present."))
    else:
        checks.append(check("SCHEMA_FILE_PRESENT", "FAIL", "Schema file is missing.", error_code="ERR_SCHEMA_FILE_MISSING", details={"schema_path": str(schema_path)}))
