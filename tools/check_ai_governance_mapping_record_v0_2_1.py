#!/usr/bin/env python3
"""
AI Governance System Mapping Record checker v0.2.1.

v0.2.1 applies precision hardening on top of v0.2:
- duplicate boundary-ID detection,
- safe-handoff self-reference guards,
- combined failure-mode coverage,
- schema version mismatch and missing schema behavior,
- malformed/non-object JSON tests,
- Unicode-aware lexical normalization for restricted-claim detection,
- preservation of v0.2 normalized result output.
...
SCHEMA_VERSION = "ai_governance_system_mapping_record.v0.2.1"
...
def check_schema_equivalent_validation(
    ...
    if not schema_path.is_file():
        add_check(result, "SCHEMA_EQUIVALENT_VALIDATION", "FAIL", "Schema file is required for v0.2.1 schema-equivalent validation.")
        return
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check AI Governance System Mapping Record v0.2.1.")
    parser.add_argument("--record", required=True, help="Path to mapping record JSON.")
    parser.add_argument("--schema", default="schemas/ai_governance_system_mapping_record_v0_2_1.schema.json")
    ...
    result: dict[str, Any] = {
        "checker_name": "check_ai_governance_mapping_record_v0_2_1",
        "checker_version": "v0.2.1",
    ...
    if schema_path.is_file():
        ...
        add_check(result, "SCHEMA_FILE_PRESENT", "FAIL", f"Schema file not found: {schema_path}. v0.2.1 requires schema-equivalent validation.")
    ...
    print(f"AI_GOVERNANCE_MAPPING_RECORD_V0_2_1_CHECK_{result['overall_status']}")
