# Independent Verification Report — Boundary-State Interoperability Checker v0.1.1 / Evidence Packet v0.1.1
...
from the uploaded zips — nothing here is taken on the word of the packaged
receipts.
...
| Standalone uploaded checker zip vs. checker embedded in evidence packet's `source_artifacts/` | **Byte-identical** (`diff -rq` empty; `check_boundary_state_interop_v0_1_1.py` sha256 `18fc4b93…1d21dcf` in both) |
...
"Byte-identical" means the full `--json` output, not just the pass count —
...
| # | Question | Answer |
|---|---|---|
| 1 | Does RWF_ADV_A now reject for the right semantic reason? | **Yes.** Rejects via `NONOPERATIVE_EXTENSION_FIELD_CONTAINS_PROTECTED_SEMANTICS` on the `x-review.review_note` text specifically — confirmed by re-running, not just reading the receipt. |
...
| 5 | Does the stronger paraphrase lint over-reject any legitimate non-operative metadata? | **Yes — demonstrated.** See §5. |
| 6 | Are there additional schema-valid paraphrases that should become reviewer-generated regression payloads? | **Yes — demonstrated.** See §5. |

Questions 1–4 were answerable from the shipped evidence plus recomputation.
Questions 5–6 were open, so I built test cases rather than reason about it
abstractly.
...
with only `handoff_id` and `extensions.x-review.review_note` changed — so any
...
### 5a. Residual paraphrase bypass class (Q6) — 5/5 constructed cases bypassed
...
- *"Nothing further stands in the way here — the team gave this a thumbs up and downstream consumers can build on it as final."*
...
- *"This one's good — ship it downstream, no notes."*
...
### 5b. Over-rejection / false-positive class (Q5) — 4/4 constructed cases wrongly rejected
...
itself). That architecture will always trade off recall against precision —
...
"scan the field value for protected semantic language" — the specific
...
triage — the same status RWF_ADV_A had before it became a v0.1.1 regression
fixture.

## Deliverables

- This report.
- `candidate_regression_payloads_v0_1_1/` — 5 bypass candidates, 4
