...
def test_admission_cannot_be_inserted_into_preserved_package(tmp_path: Path) -> None:
    root = copy_surface(tmp_path)
    (root / CHECKER.PACKAGE / "ADMISSION.json").write_text("{}\n", encoding="utf-8")
    assert "PRESERVED_PACKAGE_FILE_SET_CHANGED" in codes(CHECKER.verify(root, run_wrapper=False))
...
def test_successor_admission_index_cannot_promote_portfolio(tmp_path: Path) -> None:
    ...
    path.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    assert "PROOF_ADMISSION_INDEX_INVALID" in codes(CHECKER.verify(root, run_wrapper=False))


def test_canonical_tip_ci_observation_cannot_be_promoted_or_replaced(tmp_path: Path) -> None:
    root = copy_surface(tmp_path)
    path = root / CHECKER.OBSERVATION
    payload = json.loads(path.read_text(encoding="utf-8"))
    payload["head_sha"] = "0" * 40
    path.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    findings = CHECKER.verify(root, run_wrapper=False)
    assert "CANONICAL_CI_OBSERVATION_DIGEST_MISMATCH" in codes(findings)
    assert "CANONICAL_CI_OBSERVATION_INVALID" in codes(findings)
