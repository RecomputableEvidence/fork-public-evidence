...
def test_happy_path_outbound_eligible_subject_routes_inbound_to_outbound():
    inbound = RoutingSnapshot("OBJ-1", "R0", RoutePhase.INBOUND)
    examine = RoutingSnapshot("OBJ-1", "R1", RoutePhase.EXAMINE, predecessor_state_id="R0")
    determine = RoutingSnapshot(
        "OBJ-1", "R2", RoutePhase.DETERMINE, RouteDisposition.OUTBOUND_ELIGIBLE,
        findings=("FORMAT_OK", "SOURCE_PRESENT"), predecessor_state_id="R1"
    )
    outbound = RoutingSnapshot(
        "OBJ-1", "R3", RoutePhase.OUTBOUND, RouteDisposition.OUTBOUND_ELIGIBLE,
        findings=determine.findings, predecessor_state_id="R2"
    )
...
def test_remediation_loop_returns_to_determine_before_outbound():
    determine_1 = RoutingSnapshot("OBJ-2", "R2", RoutePhase.DETERMINE, RouteDisposition.REMEDIATION_REQUIRED, findings=("SOURCE_INCOMPLETE",))
    resolve = RoutingSnapshot("OBJ-2", "R3", RoutePhase.RESOLVE, RouteDisposition.REMEDIATION_REQUIRED, findings=("SOURCE_INCOMPLETE",), predecessor_state_id="R2")
    determine_2 = RoutingSnapshot("OBJ-2", "R4", RoutePhase.DETERMINE, RouteDisposition.OUTBOUND_ELIGIBLE, findings=("SOURCE_REESTABLISHED",), predecessor_state_id="R3")
    outbound = RoutingSnapshot("OBJ-2", "R5", RoutePhase.OUTBOUND, RouteDisposition.OUTBOUND_ELIGIBLE, findings=("SOURCE_REESTABLISHED",), predecessor_state_id="R4")
...
def test_unresolved_subject_must_enter_resolution_not_outbound():
    current = RoutingSnapshot("OBJ-3", "R2", RoutePhase.DETERMINE, RouteDisposition.UNRESOLVED)
    outbound = RoutingSnapshot("OBJ-3", "R3", RoutePhase.OUTBOUND, RouteDisposition.OUTBOUND_ELIGIBLE, predecessor_state_id="R2")
...
def test_archive_only_and_discard_only_are_terminal_routes_not_findings():
    archive = RoutingSnapshot("OBJ-4", "R2", RoutePhase.DETERMINE, RouteDisposition.ARCHIVE_ONLY)
    discard = RoutingSnapshot("OBJ-5", "R2", RoutePhase.DETERMINE, RouteDisposition.DISCARD_ONLY)
...
def test_open_finding_vocabulary_does_not_create_new_route_types():
    current = RoutingSnapshot("OBJ-6", "R2", RoutePhase.DETERMINE, RouteDisposition.REMEDIATION_REQUIRED, findings=("FILE_TRUNCATED", "OBSERVATION_TRUNCATED", "SOMETHING_NEVER_SEEN_BEFORE"))
    assert allowed_next_phases(current) == (RoutePhase.RESOLVE,)


def test_resolution_operation_vocabulary_is_open():
    current = RoutingSnapshot("OBJ-7", "R2", RoutePhase.DETERMINE, RouteDisposition.REMEDIATION_REQUIRED)
    successor = RoutingSnapshot("OBJ-7", "R3", RoutePhase.RESOLVE, RouteDisposition.REMEDIATION_REQUIRED, predecessor_state_id="R2")
    transition = _transition(current, successor, operations=("RECOMPUTE", "RECOVER", "CUSTOM_OPERATION_X"))
...
def test_direct_inbound_to_outbound_is_rejected():
    ...
    successor = RoutingSnapshot("OBJ-8", "R1", RoutePhase.OUTBOUND, RouteDisposition.OUTBOUND_ELIGIBLE, predecessor_state_id="R0")
...
def test_examination_cannot_predeclare_final_disposition():
    ...
    successor = RoutingSnapshot("OBJ-9", "R1", RoutePhase.EXAMINE, RouteDisposition.OUTBOUND_ELIGIBLE, predecessor_state_id="R0")
    violations = validate_routing_transition(current, successor, _transition(current, successor))
...
def test_determination_requires_a_disposition():
    ...
    successor = RoutingSnapshot("OBJ-10", "R2", RoutePhase.DETERMINE, predecessor_state_id="R1")
    violations = validate_routing_transition(current, successor, _transition(current, successor))
    assert "DETERMINATION_MISSING_DISPOSITION" in {v.code for v in violations}


def test_outbound_is_terminal():
    outbound = RoutingSnapshot("OBJ-11", "R3", RoutePhase.OUTBOUND, RouteDisposition.OUTBOUND_ELIGIBLE)
...
def test_subject_substitution_is_rejected():
    ...
    successor = RoutingSnapshot("OTHER", "R2", RoutePhase.DETERMINE, RouteDisposition.OUTBOUND_ELIGIBLE, predecessor_state_id="R1")
    violations = validate_routing_transition(current, successor, _transition(current, successor))
...
def test_successor_must_bind_predecessor_state():
    ...
    successor = RoutingSnapshot("OBJ-13", "R2", RoutePhase.DETERMINE, RouteDisposition.OUTBOUND_ELIGIBLE, predecessor_state_id="WRONG")
    violations = validate_routing_transition(current, successor, _transition(current, successor))
...
def test_routing_transition_requires_explicit_basis():
    ...
    successor = RoutingSnapshot("OBJ-14", "R2", RoutePhase.DETERMINE, RouteDisposition.OUTBOUND_ELIGIBLE, predecessor_state_id="R1")
...
def test_state_id_may_not_be_reused_across_temporal_transition():
    ...
    successor = RoutingSnapshot("OBJ-15", "R1", RoutePhase.DETERMINE, RouteDisposition.OUTBOUND_ELIGIBLE, predecessor_state_id="R1")
    transition = RoutingTransition("TR-1", "OBJ-15", "R1", "R1", RoutePhase.EXAMINE, RoutePhase.DETERMINE, "bounded test basis")
