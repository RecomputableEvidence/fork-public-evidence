...
CHECKER = ROOT / "tools/check_fork_self_standing_projection_v0_1_1.py"
RECORD = ROOT / (
...
def test_projection_observation_time_is_required() -> None:
    checker = load_checker()
    record = load_record()
    record["projection_observed_at"] = ""
    assert "PROJECTION_OBSERVED_AT" in codes(checker.validate_record(record))


def test_source_observation_requires_time_and_clock_domain() -> None:
    checker = load_checker()
    record = load_record()
    record["claims"][1]["evidence_provenance"][0]["observed_at"] = ""
    record["claims"][1]["evidence_provenance"][0]["clock_domain"] = ""
    found = codes(checker.validate_record(record))
    assert "EVIDENCE_OBSERVED_AT" in found
    assert "EVIDENCE_CLOCK_DOMAIN" in found


def test_independence_requires_reference_class() -> None:
    checker = load_checker()
    record = load_record()
    record["claims"][0]["independence"]["artifact_acquisition"]["reference_class"] = ""
    assert "INDEPENDENCE_REFERENCE_CLASS" in codes(checker.validate_record(record))
